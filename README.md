# Hi, I'm Ansh Mishra 👋 👨‍💻

[![Twitter Follow](https://img.shields.io/badge/follow-%40Anshuman-1DA1F2?logo=twitter&style=social)](https://x.com/anshm3491)
[![Linkedin: Ansh](https://img.shields.io/badge/-Ansh-blue?style=flat-square&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/ansh3491/)](https://www.linkedin.com/in/ansh3491/)
![GitHub followers](https://img.shields.io/github/followers/firefrost91?label=Follow&style=social)

---

## 👨🏻‍💻 About Me
![Anshuman's GitHub stats](https://github-readme-stats.vercel.app/api?username=firefrost91&show_icons=true&theme=tokyonight&show=reviews,prs_merged,prs_merged_percentage)
[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=firefrost91&layout=compact&theme=tokyonight&langs_count=8&hide=html,css,tex,less,dockerfile,makefile,shell,ejs)]


<img alt="GitHub" src="/github.svg" width="200" height="200" align="right"/>

🎓 &nbsp;Graduate student at **Carnegie Mellon University**.  
🎓 &nbsp;Previously studied **Electronics & Instrumentation at BITS Pilani**.

💼 &nbsp;Former **Machine Learning Engineer / Pod Lead at Tata AIG Innovation Labs**, where I built large-scale document intelligence and OCR systems.

🧠 &nbsp;Interested in building **AI systems, recommendation engines, distributed infrastructure, and ML platforms**.

⚡ &nbsp;Currently building and exploring:

- Recommender systems with **Kafka, Postgres, and vector search**
- **ML systems in production** (training pipelines, inference services)
- **Distributed systems and data infrastructure**
- **LLM-powered applications and AI agents**

✉️ &nbsp;Reach me at **heyansh314@gmail.com**

---

## 🚀 Tech Stack

**Languages**

Python • JavaScript • Java • SQL

**Backend & Infrastructure**

FastAPI • Flask • Node.js • Docker • Kubernetes • Kafka • gRPC

**Machine Learning**

PyTorch • TensorFlow • Scikit-learn • Vector Search • Recommender Systems

**Databases**

PostgreSQL • MongoDB • DynamoDB • Redis

**Cloud**

AWS • GCP

---

## 🔭 Current Focus

- ML Systems & Infrastructure
- Recommendation Systems
- Distributed Systems
- AI + Software Engineering

---

## 🌎 Find me around the web

- Sharing thoughts on <a href="https://www.linkedin.com/in/ansh3491/">LinkedIn</a>
- Writing occasionally on <a href="https://www.anshbuilds.com/blog">Blog</a>

---

<img src="/github-metrics.svg" alt="Metrics" width="100%">
